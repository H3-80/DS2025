...
public:
    ...
    virtual int insert(Tv const& vertex) { //插入顶点，返回编号
        for (int j = 0; j < this->n; j++) E[j].push_back(nullptr); this->n++; //各顶点预留一条潜在的关联边
        E.push_back(std::vector<Edge<Te>*>(this->n, nullptr)); //创建新顶点对应的边向量
    ...
    virtual Tv remove(int i) { //删除第i个顶点及其关联边（0 <= i < n）
        ...
        E.erase(E.begin() + i); this->n--; //删除第i行
