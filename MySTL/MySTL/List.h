...
protected:
	void init();
	int clear();
	void copyNodes(ListNodePosi(T), int);	//复制表中自位置p起的n项
...
template < typename T> //重载下标操作符，以通过秩直接讵问列表节点（虽方便，效率低，需慎用）
...
List<T>::List(ListNodePosi(T) p, int n) { copyNodes(p, n); } //复制列表中自位置p起的n顷
...
template <typename T> T List<T>::remove(ListNodePosi(T) p) { //初除合法位置p处节点，返回其数值
...
template <typename T> int List<T>::uniquify() { //成批剔除重复元素，效率更高(有序)
	if (_size < 2) return 0; //平凡列表自然无重复
	int oldSize = _size; //记录原规模
...
ListNodePosi(T) List<T>::search(T const& e, int n, ListNodePosi(T) p) const {
	...
	// assert: 至此位置p必符合输出语义约定——尽管此前最后一次兲键码比较可能没有意义（等效于与-inf比较）
		return p; //返回查找终止的位置
